...
품질 검증:
  ...
  - H2 섹션 ≥ 3개
  - 본문에 원문 출처 라인 존재
  - 외국어 CJK 잔존 문자와 AI식 번역투 반복 차단
  - (source 기반 글 필수 --source) gnomon TranslationGate: 길이 비율, 고유명사 보존

사용법:
  /opt/homebrew/bin/python3 /Users/noah/.openclaw/workspace-techllm/scripts/validate_blog.py --post content/posts/foo.md
  /opt/homebrew/bin/python3 /Users/noah/.openclaw/workspace-techllm/scripts/validate_blog.py --post content/posts/foo.md --source /tmp/source.txt --require-source
...
AI_TRANSLATION_PATTERNS = [
    ...
    ("진입하고 있음을", re.compile(r"진입하고\s*있음을|보여(?:줍니다|준다)|의미(?:합니다|한다)")),
]
...
def check_translation_naturalness(body: str) -> dict:
    foreign_hits = []
    for match in FOREIGN_CJK_RE.finditer(body):
        foreign_hits.append(
            {
                "char": match.group(0),
    ...
    mixed_english_hits = []
    for match in MIXED_ENGLISH_RE.finditer(body):
        mixed_english_hits.append(
            {
                "term": match.group(0),
                "line": line_number(body, match.start()),
            }
        )
        if len(mixed_english_hits) >= 10:
            break
    ...
    return {
        ...
        "ai_pattern_fail": ai_pattern_fail,
...
def check_quality(note: str, source_path: str = "", require_source: bool = False) -> dict:
    checks: dict = {}
    ...
    checks["source_file_present"] = bool(source_path and Path(source_path).exists())

    naturalness = check_translation_naturalness(body)
    checks["translation_naturalness"] = naturalness
    ...
    source_requirement_pass = (not require_source) or checks["source_file_present"]
    gate_pass = gate_result.get("passed") if gate_result and "passed" in gate_result else True
    naturalness_pass = naturalness["passed"]
    ...
    if require_source and not checks["source_file_present"]:
        fix_hints.append("source 기반 글은 --source 원문 텍스트 파일이 필수")
    ...
    if naturalness["foreign_cjk_hits"]:
        sample = ", ".join(
            f"{hit['char']}(line {hit['line']})"
            for hit in naturalness["foreign_cjk_hits"][:5]
        )
        fix_hints.append(f"외국어 CJK 문자 잔존: {sample}")
    if naturalness["ai_translation_hits"]:
        sample = ", ".join(
            f"{hit['pattern']} {hit['count']}회"
            for hit in naturalness["ai_translation_hits"]
        )
        if naturalness["ai_pattern_fail"]:
            fix_hints.append(f"AI식 번역투 반복 과다: {sample}")
        else:
            fix_hints.append(f"AI식 번역투 주의: {sample}")
    if naturalness["mixed_english_warning"]:
        sample = ", ".join(
            f"{hit['term']}(line {hit['line']})"
            for hit in naturalness["mixed_english_hits"][:5]
        )
        fix_hints.append(f"영어 표현 과다/미번역 의심: {sample}")

    return {
        "passed": basic_pass and source_requirement_pass and gate_pass and naturalness_pass,
        "checks": checks,
        "gate": gate_result,
        "fix_hints": fix_hints,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-source",
        action="store_true",
        help="source 기반 글에서 --source 누락을 실패로 처리",
    )
    parser.add_argument(
        "--tech-only",
        action="store_true",
    ...
    if not args.tech_only:
        result["quality"] = check_quality(text, args.source, args.require_source)
    if args.public_url:
