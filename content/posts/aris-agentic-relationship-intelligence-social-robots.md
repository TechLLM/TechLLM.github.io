...
draft: false
description: "Monash University 연구팀이 로봇이 사용자 간의 사회적 관계를 지식 그래프로建模하고 추적하는 ARIS 시스템을 개발했다. Pepper 로봇 기반 사용자 연구(N=23)에서 기존 LLM 대비 인지력, 생명성, 유사인간성, 호감도 모두 유의미하게 향상."
cover:
...
**1. 관계 데이터의 가치.** 챗봇이나 AI 어시스턴트도 결국 "누구와 대화하느냐"에 따라 대응 방식이 달라져야 한다. 사용자 간 관계를 지식 그래프로建模하는思路는 고객 응대, 멘탈 케어, 교육 같은 분야에도 적용 가능한다.

**2. 검색 증강 생성의 확장 가능성.** 천 건이든 천만 건이든 문맥을 효율적으로 검색하는 메커니즘은 모든 장기 대화 AI 시스템에 필요하다. ARIS의 하이브리드 검색 방식은方面的実装参考가 될 수 있다.
